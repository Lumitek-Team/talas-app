...
export function CommentForm({
  ...
  const onSubmit = (values: CommentFormValues) => {
    ...
    if (mode === "create") {
      ...
      form.setError("root.serverError", {
        type: "manual",
        message: "Invalid form mode or missing comment ID.",
      });
    }
  };
  ...
  if (!isUserLoaded) {
    return (
      <div className={`py-2 ${compact ? "pl-10" : ""}`}>
  ...
  return (
    <Form {...form}>
      <form
        ...
        className={`flex gap-3 items-start ${compact ? "pt-2 pb-1" : "py-4"}`}
      >
