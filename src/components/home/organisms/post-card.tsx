...
export function PostCard<T extends Partial<ProjectOneType>>({
  ...
  const postActionsVariant =
    displayContext === "saved-page" ? "bookmark-only" : "full";
  ...
  return (
    <div className={`p-4 ${isMobile ? "bg-background" : ""}`}>
      <PostHeader
        ...
        avatarSrc={
          owner && owner.photo_profile
            ? owner.photo_profile
            : "/img/dummy/avatar-dummy.jpg"
        }
        ...
        <div
          className={`grid gap-2 mb-4 ${
            allDisplayImages.length === 1
              ? "grid-cols-1"
              : allDisplayImages.length === 2
                ? "grid-cols-2"
                : allDisplayImages.length === 3
                  ? "grid-cols-3"
                  : allDisplayImages.length === 4
                    ? "grid-cols-2"
                    : "grid-cols-3"
          }`}
            ...
            return (
              <div
                ...
                className={`aspect-video bg-muted rounded-md overflow-hidden relative ${
                  allDisplayImages.length === 3 && index === 0
                    ? "md:col-span-3 row-span-2"
                    : allDisplayImages.length === 5 && index === 0
                      ? "col-span-3 md:col-span-2 row-span-2"
                      : allDisplayImages.length === 5 &&
                          (index === 1 || index === 2)
                        ? "col-span-1 md:col-span-1 row-span-1"
                        : allDisplayImages.length > 3 &&
                            index > 0 &&
                            allDisplayImages.length % 2 !== 0 &&
                            index === allDisplayImages.length - 1
                          ? "col-span-2"
                          : ""
